...
from .metrics import calc_ssim, calc_mse, calc_mae, calc_rmse, calc_psnr
...
class Experiment():
    ...
    def _acquire_device(self):
        ...
        self._use_gpu = self.args.use_gpu and (has_cuda or has_mps)

        if self.args.dist and not has_cuda:
            assert False, "Distributed training requires CUDA GPUs"
    ...
    def _preparation(self, dataloaders=None):
        ...
        if self.args.resume_from is not None:
            self._load(self.args.resume_from)
    ...
    def _load(self, load_path):
        ...
        if not isinstance(loaded_model, dict):
            raise RuntimeError(f'No state_dict found in checkpoint file {load_path}')
    ...
    def display_method_info(self):
        ...
        # Nathan
        # turning off throughput measurement
    ...
    def train(self):
        ...
        for epoch in range(self.args.epoch):
            ...
            if self._rank == 0 and self.args.checkpoint_interval is not None and (
                    epoch + 1) % self.args.checkpoint_interval == 0:
                epoch_checkpoint_path = f"{self.path}/checkpoints/epoch_{epoch + 1}.pth"
                latest_checkpoint_path = f"{self.path}/checkpoints/latest.pth"

                self._save(epoch_checkpoint_path)
                self._save(latest_checkpoint_path)

                print(f'Checkpoint reached... Saving model to {epoch_checkpoint_path}')
                print(f'Updating latest checkpoint at {latest_checkpoint_path}')
