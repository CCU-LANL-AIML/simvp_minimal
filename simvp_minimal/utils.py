...
def measure_throughput(model, input_dummy):
    ...
    total_time = 0
    with torch.no_grad():
        print(f"Starting a pass that calculates flops, throughput, etc . . .")
        print(f"This can take a while, especially on non-GPU / non-CUDA machines . . .")
        for i in range(repetitions):
            ...
            print(f"\t{i} / {repetitions}: Throughput calculation - total time: {total_time}")
